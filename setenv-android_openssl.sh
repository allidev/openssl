...
if [ -n "${PLATFORM}" -a -n "${TOOLCHAIN_VERSION}" ]; then 
  _ANDROID_EABI="${PLATFORM}-${TOOLCHAIN_VERSION}"
else 
...
if [ -n "${ANDROID_API_LEVEL}" ]; then 
  _ANDROID_API="android-${ANDROID_API_LEVEL}"
else 
...
do
  echo $ANDROID_NDK_ROOT/toolchains/$_ANDROID_EABI/prebuilt/$host/bin
